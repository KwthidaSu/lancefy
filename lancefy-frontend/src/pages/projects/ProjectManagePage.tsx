...
import type { WsEvent } from "@/types/chat.types";
...
function isChatMessageEvent(event: WsEvent): event is Message {
  return !("action" in event) || event.action === undefined;
}
...
export default function ProjectManagePage() {
  ...
  useEffect(() => {
    if (!assignment && (activeTab === "board" || activeTab === "milestones" || activeTab === "chat")) {
  ...
  const canAccessProjectChat = isOwner || isAssignee;
  const isWorkView = !isOwner && isAssignee;
    ...
    const initChat = async () => {
      try {
        ...
        if (preferredRoomId) {
          ...
          room = rooms.find((r) => r.id === preferredRoomId);
          if (!room && chatPeerId) {
            room = await chatService.createDM(chatPeerId);
          }
        } else {
          room = await chatService.createDM(chatPeerId!);
        }
        ...
        if (activeTab === "chat") {
          ...
          chatService.connect(room.id, (newMsg) => {
            if (!isChatMessageEvent(newMsg)) return;
  ...
  }, [activeTab, id, milestones, isChatRelatedTab, canAccessProjectChat, chatPeerId, preferredRoomId, t]);
  ...
  const freelancerInitial = (offer?: JobOffer) => {
    const displayName = formatFreelancerDisplayName(offer).trim();
    const firstChar = displayName.replace(/^@/, "").charAt(0).toUpperCase();
    return firstChar || "F";
  };
  ...
  const displayCurrency = project.currency ?? "THB";

  const totalBudget = payout?.total_milestone_amount ?? project.total_budget ?? project.budget ?? 0;
  ...
  const activityItems = sortByOccurredAtDesc<ActivityItem>([
    ...
    ...offers.map((offer): ActivityItem => ({
      ...
      detail: `${offer.proposed_budget?.toLocaleString?.() ?? offer.proposed_budget} ${offer.currency}`,
    ...
    <div className="p-6 w-full space-y-6 [&_button]:rounded-md">
      ...
      <MilestonePlanEditModal
        ...
        onSaved={() => {
          ...
          // refresh project to get updated milestone_plan_pending
      ...
      <div className="mb-1 overflow-x-auto">
        ...
        {[
          { key: "overview", label: t("project.managePage.tabs.overview") },
          { key: "board", label: t("project.managePage.tabs.board"), requiresAssignment: true },
          { key: "milestones", label: t("project.managePage.tabs.milestones"), requiresAssignment: true },
          { key: "chat", label: t("project.managePage.tabs.chat"), requiresAssignment: true },
          { key: "files", label: t("project.managePage.tabs.files") },
          { key: "activity", label: t("project.managePage.tabs.activity") },
        ].filter((tab) => !tab.requiresAssignment || !!assignment).map((tab) => (
          <button
            key={tab.key}
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="rounded-xl border border-border bg-white p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="font-semibold text-base">
                      {formatAmount(project.total_budget ?? project.budget)} {displayCurrency}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-text-muted">
              ...
              {(project.skill_tags ?? []).length > 0 && (
                <div className="mt-6">
                  <div className="text-sm text-text-muted mb-2">
                    {t("project.managePage.fields.skills", { defaultValue: "Skills Required" })}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {(project.skill_tags ?? []).map((skill, idx) => (
              ...
              {(project.images ?? []).length > 0 && (
                <div className="mt-6">
                  <div className="text-sm text-text-muted mb-2">
                    {t("project.managePage.fields.attachedImages", { defaultValue: "Attached Images" })}
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <div className="text-lg font-semibold">
                      {Number(totalBudget).toLocaleString()} {displayCurrency}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-text-muted">
                    ...
                    <div className="text-lg font-semibold text-blue-600">
                      {Number(funded).toLocaleString()} {displayCurrency}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-text-muted">
                    ...
                    <div className="text-lg font-semibold text-lime-600">
                      {Number(released).toLocaleString()} {displayCurrency}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-text-muted">
                    ...
                    <div className="text-lg font-semibold text-orange-600">
                      {Number(remaining).toLocaleString()} {displayCurrency}
                    </div>
                  </div>
                </div>
              )}
            ...
            {!isAssignee && isOwner && (
              <div className="rounded-xl border border-border bg-white p-6">
                <h2 className="flex items-center gap-2 text-lg font-semibold mb-4 text-primary-foreground">
                  <HiOutlineRocketLaunch className="w-5 h-5" />{" "}
                  {t("project.managePage.offers.title")}
                </h2>
                    ...
                    {offers.map((offer) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-between items-start mb-2">
                            <div className="flex items-center gap-2">
                              <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-base">
                                {freelancerInitial(offer)}
                              </div>
                              <div>
                                <div className="text-base font-bold text-gray-900">
                                  {formatFreelancerDisplayName(offer)}
                                </div>
                            ...
                            <div className="text-right">
                              <div className="text-lg font-bold text-gray-900">
                                ฿{formatAmount(offer.proposed_budget)}
                              </div>
                          ...
                          {offer.proposed_milestones?.length > 0 && (
                            <div className="mb-4 rounded-xl border border-blue-100 bg-gradient-to-b from-blue-50/70 to-white p-4">
                              <div className="mb-3 text-sm font-semibold text-blue-900">
                                Milestones ที่เสนอ ({offer.proposed_milestones.length})
                              </div>
                              <div className="space-y-3">
                                {offer.proposed_milestones.map((m, idx) => (
                                  <div
                                    key={m.id}
                                    ...
                                    <div className="flex items-start justify-between gap-3">
                                      <div className="min-w-0 flex-1">
                                        ...
                                        {m.estimated_days != null && (
                                          <div className="mt-2 inline-flex rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700">
                                            ระยะเวลา {m.estimated_days} วัน
                                          </div>
                                        )}
                                      </div>
                                      <div className="shrink-0 text-base font-bold text-blue-700">
                                        ฿
                          ...
                          <div className="flex gap-2 flex-wrap">
                            {offerStatus === "pending" &&
                              offer.offer_type !== "counter_offer" && (
                              <div className="w-full flex gap-2">
                                <button
                                  onClick={() => handleOpenAcceptOffer(offer.id)}
                                  className="flex-1 bg-blue-600 text-white py-2 rounded-lg text-xs font-bold hover:bg-blue-700 hover:shadow-lg transition-all"
                                >
                                  {t("project.managePage.offers.accept")}
                                </button>
                                <button
                                  onClick={() => handleOpenCounterOffer(offer.id)}
                                  className="flex-1 bg-amber-500 text-white py-2 rounded-lg text-xs font-bold hover:bg-amber-600 transition-all"
                                >
                                  {t("project.managePage.offers.counter")}
                                </button>
                                <button
                                  onClick={() => handleOpenRejectOffer(offer.id)}
                                  className="flex-1 bg-rose-600 text-white py-2 rounded-lg text-xs font-bold hover:bg-rose-700 transition-all"
                                >
                            ...
                            {offerStatus === "pending" &&
                              offer.offer_type === "counter_offer" && (
                                <div className="w-full space-y-2">
                                  <div className="w-full text-center py-2 bg-amber-50 text-amber-700 rounded-lg text-xs font-bold border border-amber-200">
                                    {t("project.managePage.offers.counterPending")}
                                  </div>
                                  <div className="w-full flex gap-2">
                                    <button
                                      onClick={() => handleOpenAcceptOffer(offer.id)}
                            ...
                            {offerStatus === "accepted" && (
                              <div className="w-full text-center py-2 bg-lime-50 text-text-primary rounded-lg text-xs font-bold border border-lime-200">
                            ...
                            <div className="w-full">
                              <button
                                onClick={() =>
                                  handleChatWithFreelancer(offer.freelancer_id)
                                }
                                className="w-full px-4 py-2 border border-border rounded-lg text-xs font-medium hover:bg-gray-50"
                              >
              ...
              <div className="rounded-xl border border-border bg-white p-6">
                ...
                {isOwnOfferPending || isOwnOfferAccepted ? (
                  ...
                  <button
                    ...
                    disabled={!canOffer}
                    className="w-full flex items-center justify-center gap-3 px-4 py-2.5 rounded-lg border border-accent/30 bg-accent/10 text-primary-foreground hover:bg-accent/15 transition disabled:opacity-60"
                  >
                    <HiOutlineRocketLaunch className="w-5 h-5" />
                    {t("project.offer")}
                  </button>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {w && (
                      <span
                        className={`px-2.5 py-1 rounded-full border text-[12px] font-semibold ${w.className}`}
                      >
                        {w.label}
                      </span>
                    )}
                    {f && (
                      <span
                        className={`px-2.5 py-1 rounded-full border text-[12px] font-semibold ${f.className}`}
                      >
                        {f.label}
                      </span>
                    )}
                    {s && (
                      <span
                        className={`px-2.5 py-1 rounded-full border text-[12px] font-semibold ${s.className}`}
                      >
                        {s.label}
                      </span>
                    )}
